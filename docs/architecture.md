...
| **8 of 38 have no cook card** | ✅ Closed for everything that can be closed: all 30 recipes with a `method` now carry one. The remaining 8 have no method to restructure — 4 retired, 2 generated stubs, plus the toast-and-bagels breakfast and the emergency meal. Writing cards for those means writing the recipes first, which is stage A's job. |
